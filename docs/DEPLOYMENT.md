...
> 2026-04-29 갱신 — 보안 게이트 / 일일 P&L / 종목+거래 통합 / 자동 수수료 /
> 폰트 마라톤 (40라운드) / 모바일 stacking 모두 완료. 운영 안정 단계.
...
### ~~3) 보안 — 컨테이너 인증 없음~~ ✅ 해결 (2026-04-27)
DB 기반 bcrypt 비밀번호 게이트 도입. 첫 접속 시 사용자가 직접 비밀번호 등록,
이후 로그인. 환경변수에 평문 노출 없음 (운영자도 비밀번호 모름). 두 인스턴스
각자 별도 비밀번호.
- `src/auth.py`: `is_password_set()`, `set_password()`, `verify_password()`,
  `change_password()`, `require_auth()` (등록/로그인 UI), `render_sidebar_change_password()`
- `auth` 테이블 추가 (`auth_set_hash`/`auth_get_hash`/`auth_clear`)
- 사이드바에 "🔐 비밀번호 변경" expander
- bcrypt rounds=12

### ~~4) 폰트 미세조정 (40라운드 마라톤)~~ ✅ 해결 (2026-04-29)
"숫자가 한 글자씩 흩어져 보임" 문제 — 처음엔 폰트 자간 이슈로 추정했으나
40라운드 추격 끝에 **진짜 진범 두 개**:
1. **Streamlit markdown sanitizer 가 inline style 속성을 strip** (라운드 32 발견).
   라운드 28~31의 모든 inline 시도가 무력화됐던 진짜 이유. 클래스 기반 CSS +
   고 specificity (`.cm-value .cm-latin` = 20) 으로 해결.
2. **모바일에서 4-col / 3-col 레이아웃이 그대로 유지되어 카드당 80px** —
   9자리 숫자 절대 안 들어감 (라운드 39~40 발견). `@media (max-width: 1024px)`
   에서 `[data-testid="stHorizontalBlock"]` flex-direction 을 column 으로 강제.

부산물:
- `src/ui_components.py` — `metric()` HTML drop-in (st.metric 의 emotion CSS
  우회). `_split_by_script()` 가 Latin/Hangul 을 별도 span 으로 분리해 Chrome
  CJK shaper 의 ASCII 전각 강제 차단.
- `style.py` 의 `.cm-metric` / `.cm-latin` / `.cm-hangul` 클래스 + Oswald 폰트
  (Google Fonts CDN, 신문 헤드라인 스타일의 narrow 디자인).
- 데스크탑 4-col / 3-col 유지, 모바일/태블릿 (≤1024px) 자동 stacking.

### ~~5) 모바일 메트릭 라벨 잘림~~ ✅ 해결 (2026-04-29)
라운드 40 의 모바일 stacking 으로 자동 해결. 카드가 화면 가로폭 다 차지해서
라벨이 한 줄에 깔끔히 들어감.

### 6) NAS GitHub SSH 키 셋업 (보류)
- 현재 wget+tar 방식 + `update.sh` 스크립트로 충분히 운영 가능
- `sudo bash /volume1/docker/update.sh all` 한 줄로 두 인스턴스 동시 갱신
- 우선순위 낮음

### 7) 향후 Phase
- Phase 2 — CSV 임포터: 사용자가 수동 입력 선호로 **보류 (실제 필요 X)**
- Phase 2 — yfinance/pykrx 과거 시세 백필: 사용자가 분할매수 + 정확한 매수일
  입력 부담으로 **보류**
- Phase 3 — 화면 구현 (시세 후 차트/계좌별/요약 강화) ✅ 대부분 완료
- Phase 4 — 세금 모듈 마무리 + CSV 내보내기 (corp 인스턴스 전용)

### ~~8) "초기 보유분" 폼에 수수료 필드 추가~~ ✅ 해결 (2026-04-28)
계좌 관리에 `default_fee_rate` 컬럼 도입. 거래 입력 시 자동으로
`수량 × 단가 × default_fee_rate% × 환율` 로 수수료 계산되어 박힘.
사용자가 직접 수수료를 넣으면 그 값이 우선. `db.compute_auto_fee()` 헬퍼 +
6 케이스 단위테스트.

### 9) 차트 가로축 — 과거 시세 백필 (보류)
사용자가 분할매수 + 매수일 정확 입력 부담으로 보류. 백필 자체 코드는 yfinance
`Ticker.history(start, end)` + pykrx `get_market_ohlcv_by_date` 로 직선 작업.

### 10) 회계 메트릭 디자인 — 분배금/예수금 (분배금 ✅ / 예수금 보류)
- **분배금**: ✅ "💎 총수익 (분배금 포함, 세후)" 카드 추가 (2026-04-28)
  미실현 손익 + 누적 분배금(세후) 합산. 산만함 줄이기 위해 카드 한 개로 통합.
- **예수금 (cash_balance)**: 보류 — 입출금 입력 부담으로 사용자가 후순위 선언.

### 11) 일일 P&L 메트릭 ✅ 해결 (2026-04-28)
"오늘 변동 +X원 (+Y%)" 카드 추가. yfinance `previousClose` + pykrx 직전
거래일 종가 활용. `price_snapshots.previous_close` 컬럼 마이그레이션.
요약 페이지 상단 카드 + 계좌별 요약 카드 양쪽에 표시.
"기준 04-28 21:35 (라이브 갱신)" 식의 시간 캡션. 6 단위테스트.

### 12) 종목 + 거래 페이지 통합 ✅ 해결 (2026-04-28)
사용자 요청 "종목관리와 거래입력 굳이 따로 할 필요없지않아??". 두 메뉴를
`📦 종목 + 거래` 한 페이지로 통합. 매수하면 보유 종목 목록에 자동 등록.
하단에 보유 종목 표 (보유량 / 현재가 / 평가금액 / 미실현손익 / 수익률 / 시세
상태) + 0주 종목 자동 숨김.
